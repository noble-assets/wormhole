...
func (m IBCModule) OnChanOpenInit(ctx sdk.Context, _ channeltypes.Order, _ []string, port string, _ string, _ channeltypes.Counterparty, version string) (string, error) {
	if port != types.Port {
...
func (m IBCModule) OnChanOpenTry(ctx sdk.Context, _ channeltypes.Order, _ []string, port string, _ string, _ channeltypes.Counterparty, counterpartyVersion string) (string, error) {
	if port != types.Port {
		return "", errors.Wrapf(types.ErrInvalidPort, "expected port %s, got %s", types.Port, port)
	}
...
func (m IBCModule) OnRecvPacket(_ sdk.Context, _ string, _ channeltypes.Packet, _ sdk.AccAddress) exported.Acknowledgement {
	return channeltypes.NewErrorAcknowledgement(fmt.Errorf("channels with version %s cannot receive packets", types.Version))
}

func (m IBCModule) OnAcknowledgementPacket(_ sdk.Context, _ string, _ channeltypes.Packet, _ []byte, _ sdk.AccAddress) error {
	return nil
}

func (m IBCModule) OnTimeoutPacket(_ sdk.Context, _ string, _ channeltypes.Packet, _ sdk.AccAddress) error {
	return nil
}
